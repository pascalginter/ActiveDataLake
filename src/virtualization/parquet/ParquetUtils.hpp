...
public:
    ...
    static std::vector<uint8_t> writePageWithoutData(uint64_t uncompressed_size, uint64_t num_values){
        ...
        std::cout << "uncompressed page size " << uncompressed_size << std::endl;
        result.push_back(NEXT_INTEGER_FIELD);
        appendZigZagVarint(result, uncompressed_size);
        ...
        // Write num_values
        result.push_back(NEXT_INTEGER_FIELD);
        appendZigZagVarint(result, num_values);
        // Write encoding
        result.push_back(NEXT_INTEGER_FIELD);
        appendZigZagVarint(result, PLAIN_ENCODING);
        // Write definition level encoding
        result.push_back(NEXT_INTEGER_FIELD);
        appendZigZagVarint(result, RLE_ENCODING);
